pipeline {
  ...
  parameters {
    string(name: 'GLUON_VERSION', defaultValue: 'v2018.2', description: 'Gluon version')
    string(name: 'BUILD_TYPE', defaultValue: 'experimental', description: 'experimental, beta, stable')
    string(name: 'VERSION', defaultValue: '1.4.6', description: 'Firmware version')
  }
  stages {
    stage('prepare build') {
      steps {
    ...
    stage("build ipq40xx") {
      steps {
        dir("/temp/${env.BRANCH_NAME}") {
          sh "make -j13 V=s GLUON_TARGET=ipq40xx GLUON_BRANCH=${params.BUILD_TYPE} GLUON_RELEASE=${VERSION}${BUILD_TYPE}_`date '+%Y%m%d'`"
        }
      }
    }
    stage("build ipq806x") {
    ...
    stage('build ramips-mt7620') {
      steps {
        dir("/temp/${env.BRANCH_NAME}") {
          sh "make -j13 V=s GLUON_TARGET=ramips-mt7620 GLUON_BRANCH=${params.BUILD_TYPE} GLUON_RELEASE=${VERSION}${BUILD_TYPE}_`date '+%Y%m%d'`"
        }
      }
    }
    ...
    stage('build ramips-mt76x8') {
      steps {
        dir("/temp/${env.BRANCH_NAME}") {
          sh "make -j13 V=s GLUON_TARGET=ramips-mt76x8 GLUON_BRANCH=${params.BUILD_TYPE} GLUON_RELEASE=${VERSION}${BUILD_TYPE}_`date '+%Y%m%d'`"
        }
      }
    }
    stage('build ramips-rt305x') {
      steps {
        dir("/temp/${env.BRANCH_NAME}") {
          sh "make -j13 V=s GLUON_TARGET=ramips-rt305x GLUON_BRANCH=${params.BUILD_TYPE} GLUON_RELEASE=${VERSION}${BUILD_TYPE}_`date '+%Y%m%d'`"
        }
      }
    }
    stage('build sunxi') {
